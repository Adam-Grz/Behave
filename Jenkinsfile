          ...
          script {
            parallel (
                ...
                "javascript" : {
                    ...
                    sh 'docker exec -d jsAG /bin/bash -c "java -jar selenium-server-standalone-2.39.0.jar -host localhost -port 4444 > /dev/null"'
                    sh 'docker exec jsAG /bin/bash -c "cd JavaScript; sudo protractor run.js"'
